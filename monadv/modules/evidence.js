export default async function render(el, { cdn, store }) {
  el.innerHTML = `
    <div class="monadv-ev">
      ...
      <label><input type="checkbox" data-k="expert"> Экспертиза</label><br>
      <div id="out" style="margin-top:.75rem"></div>
      <button id="copyBtn" style="margin-top:.5rem">Скопировать</button>
    </div>
  `;
  ...
  el.querySelector('#copyBtn').addEventListener('click',()=>{
    ...
    el.querySelector('#copyBtn').textContent='Скопировано';
    setTimeout(()=>el.querySelector('#copyBtn').textContent='Скопировать',1200);
  });
  refresh();
}